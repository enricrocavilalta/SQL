...
select departamento.nombre as departament, persona.nombre, apellido1, apellido2 from profesor left join departamento on profesor.id_profesor=departamento.id inner join persona on profesor.id_profesor=persona.id;
...
select count(*),nombre from profesor inner join departamento on profesor.id_departamento=departamento.id group by nombre order by count(*) desc;
select count(*),nombre from profesor inner join departamento on profesor.id_departamento=departamento.id group by nombre order by count(*) desc;
...
select grado.nombre, tipo, count(*) from grado left join asignatura on grado.id=asignatura.id_grado group by grado.nombre,tipo order by nombre;
select persona.id, persona.nombre, apellido1, apellido2, count(*)-1 as asignaturas from profesor inner join persona on profesor.id_profesor=persona.id left join asignatura on profesor.id_profesor=asignatura.id_profesor group by asignatura.id_profesor order by count(*) desc;
